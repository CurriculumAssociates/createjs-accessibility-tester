...
export default class SingleLineTextInput extends createjs.Container {
  ...
  _onCanvasKeyDown(evt) {
    ...
    if ((evt.key >= 'a' && evt.key <= 'z') || (evt.key >= '0' && evt.key <= '9')) {
      ...
      if (evt.keyCode === KeyCodes.right) {
        this._selection.start = this._selection.start === -1 ? this._cursorIndex
  ...
  _cursorToIndex() {
    this._cursor.x = this._text._getMeasuredWidth(this._text.text.substring(0, this._cursorIndex))
    + this._text.x;
  }
